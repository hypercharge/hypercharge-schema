...
describe 'RecurringGtdSepaDebitSale JSON Schema' do
  let(:schema_path){ schema_path_for('recurring_gtd_sepa_debit_sale') }
  subject{ fixture('recurring_gtd_sepa_debit_sale') }
  let(:root_key){ subject.keys.first }

  root_wont_allow_additional_properties
  spec_attribute 'transaction_type', value: 'recurring_gtd_sepa_debit_sale'
