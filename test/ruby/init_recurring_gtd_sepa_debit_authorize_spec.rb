...
describe 'InitRecurringGtdSepaDebitAuthorize JSON Schema' do
  let(:schema_path){ schema_path_for('init_recurring_gtd_sepa_debit_authorize') }
  subject{ fixture('init_recurring_gtd_sepa_debit_authorize') }
  let(:root_key){ subject.keys.first }

  root_wont_allow_additional_properties
  spec_attribute 'transaction_type', value: 'init_recurring_gtd_sepa_debit_authorize'
