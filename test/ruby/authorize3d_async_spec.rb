...
describe 'Authorize3d Async JSON Schema' do
  let(:schema_path){ schema_path_for('authorize3d') }
  subject{ fixture('authorize3d_async') }
  let(:root_key){ subject.keys.first }

  root_wont_allow_additional_properties
  spec_attribute 'transaction_type', value: 'authorize3d'
