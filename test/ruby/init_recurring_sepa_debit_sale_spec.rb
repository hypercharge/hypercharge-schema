...
describe 'InitRecurringSepaDebitSale JSON Schema' do
  let(:schema_path){ schema_path_for('init_recurring_sepa_debit_sale') }
  subject{ fixture('init_recurring_sepa_debit_sale') }
  let(:root_key){ subject.keys.first }

  root_wont_allow_additional_properties
  spec_attribute 'transaction_type', value: 'init_recurring_sepa_debit_sale'
